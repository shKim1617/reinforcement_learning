...
class DroneFormationEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.leader.update_position(np.array([1.0, 0.0], dtype=np.float32))  # 리더는 고정된 방향으로 이동한다고 가정
    ...
    def _compute_reward(self):
        # Get leader's heading (assume it's a 2D orientation vector or angle)
        leader_heading = self.leader.orientation  # A unit vector representing direction

        # Define the original target positions for an A-shaped formation relative to the leader (before rotation)
        original_target_follower1 = np.array([10, 10], dtype=np.float32)  # 10 units right and 10 units up
        original_target_follower2 = np.array([10, -10], dtype=np.float32)  # 10 units right and 10 units down

        # Calculate the rotation matrix based on the leader's heading (orientation vector)
        rotation_matrix = self.leader._rotation_matrix()  # Rotation matrix based on leader's heading

        # Rotate the target positions relative to the leader's heading
        ...
        # Calculate the global target positions for the followers by adding leader's position
        ...
        # Calculate the distances from the actual positions to the rotated target positions
        ...
        # Penalize deviation from the formation (minimize distance)
        # The closer the followers are to their target positions, the higher the reward
        reward = - (distance_follower1 + distance_follower2)  # Penalize for being far from the target

        # Optionally, add a strong penalty if a collision occurs
        if self._check_collision():
            reward -= 100  # Add a large penalty for collision
...
# PPO 모델 학습
model = PPO("MlpPolicy", env, verbose=1, device=device)  # CUDA가 가능한 경우 GPU 사용

# 모델 학습
model.learn(total_timesteps=10000)                                                          # 모델 학습은 왜 여기 있어....
