...
class DroneFormationEnv(gym.Env):
    def __init__(self):
        ...
        # 리더와 팔로우 드론 관측 공간을 각각 따로 생성
        self.observation_spaces = [observation_space for _ in range(NUM_FOLLOWS)]
        
        # 행동 공간: 팔로워 드론들의 이동 방향 (예: [dx, dy]로 각 드론의 이동을 정의)
        action_space = gym.spaces.Box(low=-1.0, high=1.0, shape=(2,), dtype=np.float32)
        self.action_spaces = [action_space for _ in range(NUM_FOLLOWS)]
    ...
    def step(self, action_spaces):
        ...
        for i in range(NUM_FOLLOWS):
            self.follows[i].move(action_spaces[i])
    ...
    def _check_too_far(self):
        ...
        rewards = []
        for i in range(NUM_FOLLOWS):
            if self.follows[i].distance_to_leader > MAX_DISTANCE:
                rewards.append(-1)
            else:
                rewards.append(0)
    ...
    def _is_done(self):
        ...
        if dist_follows > 100 or self.current_step > MAX_STEPS:
